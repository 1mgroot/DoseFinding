...
test_that("PoC calibration framework works correctly", {
  ...
  # Check normalized calibration results table
  ...
  expect_true(results$achieved_rate >= 0 && results$achieved_rate <= 1)
...
test_that("Early termination calibration framework works correctly", {
  ...
  # Check calibration results data frame. Quick calibration currently searches a c_T/c_E grid.
  expect_true(is.data.frame(results$calibration_results))
  expect_true(nrow(results$calibration_results) > 0)
  expect_true(all(c("c_T", "c_E", "termination_rate") %in%
                 colnames(results$calibration_results)))

  # Check optimal values are reasonable
  expect_true(results$optimal_c_T >= 0.6 && results$optimal_c_T <= 0.9)
  expect_true(results$optimal_c_E >= 0.6 && results$optimal_c_E <= 0.9)
  expect_true(results$optimal_rate >= 0 && results$optimal_rate <= 1)
  expect_equal(results$target_rate, 0.80)
  expect_equal(results$threshold_type, "c_T & c_E")
})
