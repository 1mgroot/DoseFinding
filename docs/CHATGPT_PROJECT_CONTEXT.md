...
- `poc_calibration_new.R`：较新的 PoC 校准系统；
- `poc_calibration.R`：较早的 PoC 校准版本；
- `early_termination_calibration.R`：早停校准；
- `parameter_optimization.R`：参数搜索；
- `run_optimization.R`：优化运行入口。

注意：当前同时存在 `poc_calibration.R` 和 `poc_calibration_new.R`，需要确认哪个是正式 canonical 版本。
...
当前问题：直接运行 `testthat::test_dir("tests")` 会有部分测试因为 source 路径问题失败。
...
旧文档里有些写项目 100% 完成，但另一些文档仍写 pending。这些旧总结已经整理到 `docs/archive/`，当前优先以 `docs/README.md`、本文档和 `STAT_METHODS_AS_BUILT.md` 为准。

README 之前残留过 Git 合并冲突标记；在本次文档整理中已经清理。
...
时，部分测试失败。主要原因不是算法逻辑错，而是测试文件使用了类似 `source("src/core/...")` 的项目根目录相对路径；`test_dir()` 运行时当前目录变化，导致找不到文件。
...
- 清理 README 合并冲突；
- 修复测试 source 路径；
- 确认 `poc_calibration_new.R` 是否为正式版本；
- 同步 README、状态文档、运行指南。
...
回答：核心模拟系统已经实现并能跑通，但项目还需要清理文档、修测试路径、统一 PoC 定义和目标、运行生产规模 operating characteristics。

### 最大风险是什么？

回答：不是主流程不能跑，而是统计规则和文档状态还没完全统一，特别是 PoC 公式、PoC 校准目标、是否支持对照臂这几个问题。
