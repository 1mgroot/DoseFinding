...
\begin{enumerate}
  ...
  \item When trial stops with no early stopping, identify the final OD:
  \begin{align}
    OD=\arg\max_{j\in A}\ \hat{U}(d_j).
    \label{eq:OD}
  \end{align}
  First construct PoC:
  \begin{align}
    Pr(\pi_{I1}<\delta\,\pi_{Ij}\mid D_n) > C_{\text{PoC}}.
    \label{eq:PoC}
  \end{align}
  pass $\Rightarrow$ select final OD.
\end{enumerate}
