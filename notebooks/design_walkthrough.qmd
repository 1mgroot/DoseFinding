...
This notebook is an explanatory walkthrough, not a routine run notebook. Open it when you want to understand how the design maps to the implementation; routine users should start with `simulation_notebook.qmd`, then use the calibration notebooks as needed.

The numerical output in this walkthrough uses the standalone demonstration
defaults from `src/core/config.R`. It does not load the saved production
calibration RDS files. Use the threshold calibration, PoC calibration,
simulation, and scenario-comparison notebooks for current calibrated values and
production results.
...
# 2. Given Y_I, generate (Y_T, Y_E) from the joint-cell generator with:
#    - Conditional endpoint probabilities: p_YT_given_I[dose, Y_I],
#      p_YE_given_I[dose, Y_I]
#    - rho0/rho1; rho=0 gives the TeX conditional-independence design
...
cat("  Safety: Pr(π_T < ", trial_config$phi_T, " | data) > ", trial_config$c_T, "\n")
cat("  Activity: Pr(π_I > ", trial_config$phi_I, " | data) > ", trial_config$c_I, "\n")
cat("  Efficacy: Pr(π_E > ", trial_config$phi_E, " | data) > ", trial_config$c_E, "\n\n")
...
Adaptive randomization assigns patients based on posterior probability of being optimal among the current admissible doses:

$$
\Pr\{d_j = \arg\max_{k \in A} U_k^{(s)} \mid \text{posterior data}\}
$$

where $A$ is the current admissible set and $U_k^{(s)}$ is the draw-level utility for dose $k$. Ties are split equally within each posterior draw.
...
# 1. Compute draw-level utility for each admissible dose
# 2. Estimate posterior probability that each dose is optimal
...
ggplot(trial_results$all_alloc_probs, aes(x = Stage, y = Prob, color = factor(Dose), group = Dose)) +
  ...
  labs(title = "Adaptive Allocation Probabilities Over Time",
       subtitle = "Probabilities shift toward doses more often optimal in posterior draws",
       x = "Stage",
...
cat("\nAllocation probability changes (Stage 1 → Final):\n")
...
The implementation follows the Design2 PoC set directly:

- For each admissible dose $j$, compute: $\Pr(\pi_{I1} < \delta \cdot \pi_{Ij} \mid D_n)$
- Build $\mathcal{P}$ from doses whose probability is greater than `c_poc`
- If $\mathcal{P}$ is empty, no final OD is selected
- If $\mathcal{P}$ is non-empty, select the highest-utility dose from $\mathcal{P}$

```{r show-poc, eval=FALSE}
# PoC validation
# See src/decision/dose_decision.R: select_final_od_with_poc()

# 1. Compare each admissible dose against dose 1 using immune-response samples
# 2. Keep doses with Pr(pi_I1 < delta_poc * pi_Ij | D_n) > c_poc
# 3. Select the highest-utility dose from that PoC-eligible set
# 4. If no dose passes PoC, return NA (no final OD selected)
```
...
if (!trial_results$terminated_early) {
  ...
  if (trial_results$poc_validated) {
    cat("\n✓ PoC VALIDATED: Selected dose shows convincing immune response advantage\n")
  } else {
    cat("\n⚠ PoC NOT VALIDATED: Evidence insufficient for confident selection\n")
  }
}
```
...
#   - Sample size = (termination_stage) × (cohort_size)
```
...
if (toxic_trial$terminated_early) {
  cat("✓ Trial terminated early (as expected for toxic scenario)\n")
...
### From Design1.tex:
1. **Section 1**: Mediator factorization $\Pr(Y_I, Y_T, Y_E \mid d)$ ✅
2. **Section 2**: Beta-Bernoulli immune response model with PAVA ✅
3. **Section 3**: Toxicity conditional on mediator with BIVISO ✅
4. **Section 4**: Efficacy conditional on mediator with BIVISO ✅
5. **Section 5**: Marginalization over mediator ✅

### From Design2.tex:
1. **Section 1.1**: Utility table and expected utility calculation ✅
2. **Section 1.3**: Admissible set criteria (equations 3-5, no control) ✅
3. **Section 2**: Group sequential trial design ✅
   - Equal randomization in Stage 1 ✅
   - Adaptive randomization in subsequent stages ✅
   - Early termination when $A = \emptyset$ ✅
   - PoC-eligible set and final selection ✅
4. **Notes**: Utility-based adaptive allocation ✅

## ⚠️ OUT-OF-SCOPE Components

### From Design2.tex:
1. **Section 1.3**: Control arm criteria (equations 6-8) ❌
2. **Notes**: Control arm allocation logic (equations 11-12) ❌
...
3. **Adaptive design**: Allocation shifts toward doses with higher posterior probability of being optimal
...
1. ✅ All statistical models correctly implement the design equations
2. ✅ Isotonic regression ensures monotonicity constraints
3. ✅ Utility-based adaptive allocation shifts resources to promising doses
4. ✅ Multi-criteria admissibility screening ensures safety, activity, and efficacy
5. ✅ PoC validation provides confidence in final selection
6. ✅ Early termination prevents futile enrollment
7. ✅ Posteriors accurately recover true simulation parameters
