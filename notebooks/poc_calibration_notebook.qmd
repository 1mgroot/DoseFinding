...
**Example**: We set `null_p_I = 0.25` (true value) \> `phi_I = 0.20` (threshold) to compensate for PAVA bias.

| Parameter Type | Immune | Efficacy | Toxicity | Purpose |
|---------------|---------------|---------------|---------------|---------------|
| **Null Scenario** | null_p_I = 0.25 | null_p_E = 0.30 | null_p_T = 0.05 | Generate data |
| **Thresholds** | phi_I = 0.20 | phi_E = 0.25 | phi_T = 0.30 | Make decisions |
| **Relationship** | null_p_I \> phi_I | null_p_E \> phi_E | null_p_T \< phi_T | By design |

```{r config}
# Calibration parameters per email specification
# Adjusted: null scenario true values set ABOVE thresholds to compensate for PAVA bias
calibration_params <- list(
  null_p_I = 0.25,      # NULL SCENARIO: True immune response rate (set ABOVE threshold phi_I=0.20)
  null_p_E = 0.30,      # NULL SCENARIO: True efficacy rate (set ABOVE threshold phi_E=0.25)
