...
threshold_settings <- list(
  ...
  c_T_candidates = if (quick_mode) c(0.45, 0.55, 0.65) else seq(0.45, 0.65, by = 0.05),
  c_E_candidates = if (quick_mode) c(0.45, 0.50, 0.55) else seq(0.45, 0.65, by = 0.05),
  c_I_candidates = if (quick_mode) c(0.60, 0.70, 0.80) else seq(0.60, 0.80, by = 0.05),
  n_sim_per_candidate = if (quick_mode) 5 else 500,
  validation_n_sim = if (quick_mode) 5 else 500
)
```
...
phi_T <- threshold_settings$phi_T
phi_E <- threshold_settings$phi_E
phi_I <- threshold_settings$phi_I

make_profile <- function(start, end) {
  seq(start, end, length.out = length(threshold_settings$dose_levels))
}

make_conditional_matrix <- function(i0_start, i0_end, i1_start, i1_end) {
  matrix(
    c(make_profile(i0_start, i0_end), make_profile(i1_start, i1_end)),
    nrow = length(threshold_settings$dose_levels),
    ncol = 2
  )
}

# Base trial configuration
create_base_config <- function(c_T = 0.55, c_E = 0.50, c_I = 0.70) {
  config <- list(
    dose_levels = threshold_settings$dose_levels,
    n_stages = threshold_settings$n_stages,
    cohort_size = threshold_settings$cohort_size,
    ...
    c_poc = threshold_settings$c_poc,
    delta_poc = threshold_settings$delta_poc,
...
cat("  Simulations per candidate:", threshold_settings$n_sim_per_candidate, "\n")
```
...
create_high_tox_scenario <- function() {
  ...
  p_YT_given_I <- make_conditional_matrix(0.32, 0.50, 0.35, 0.55)
  
  # Efficacy meets criteria (all > phi_E = 0.25, increasing)
  p_YE_given_I <- make_conditional_matrix(0.30, 0.50, 0.35, 0.55)
  
  # Immune response meets criteria (all > phi_I = 0.20, increasing)
  p_YI <- make_profile(0.25, 0.45)
...
create_low_eff_scenario <- function() {
  ...
  p_YT_given_I <- make_conditional_matrix(0.10, 0.26, 0.12, 0.28)
  
  # Efficacy slightly below threshold (approaching phi_E = 0.25, increasing)
  p_YE_given_I <- make_conditional_matrix(0.12, 0.22, 0.14, 0.24)
...
calibrate_threshold <- function(
  ...
  scenario,
  candidates,
  fixed_params = list(c_T = 0.55, c_E = 0.50, c_I = 0.70),
  n_sim = 500,
  target_esr_range = c(0.60, 0.70)
) {
  cat("\n", rep("=", 60), "\n", sep = "")
...
c_T_candidates <- threshold_settings$c_T_candidates
...
c_T_calibration <- calibrate_threshold(
  ...
  fixed_params = list(c_T = 0.55, c_E = 0.50, c_I = 0.70),
  n_sim = threshold_settings$n_sim_per_candidate,
  target_esr_range = threshold_settings$target_esr_range
)
```
...
c_T_df <- data.frame(
  ...
  Status = sapply(c_T_calibration$results, function(x) {
    if (x$esr >= threshold_settings$target_esr_range[1] &&
        x$esr <= threshold_settings$target_esr_range[2]) "Within target" else ""
  }),
  check.names = FALSE
)
...
c_E_candidates <- threshold_settings$c_E_candidates

c_E_calibration <- calibrate_threshold(
  param_name = "c_E",
  scenario = low_eff,
  candidates = c_E_candidates,
  fixed_params = list(c_T = c_T_calibration$optimal_value, c_E = 0.50, c_I = 0.70),
  n_sim = threshold_settings$n_sim_per_candidate,
  target_esr_range = threshold_settings$target_esr_range
)
```
...
c_I_candidates <- threshold_settings$c_I_candidates
...
#| echo: false
target_label <- paste0(
  threshold_settings$target_esr_range[1] * 100,
  "-",
  threshold_settings$target_esr_range[2] * 100,
  "%"
)
cat("\n")
cat(rep("=", 70), "\n", sep = "")
...
for (name in names(scenarios)) {
  ...
  sim_res <- run_calibration_simulations(
    calibrated_config,
    scenarios[[name]],
    n_sim = threshold_settings$validation_n_sim
  )
...
cat("  ", rep("-", 45), "\n", sep = "")
cat(sprintf("  High Toxicity    |  %.1f%%    |  ~%s (c_T dominant)\n",
            validation_results$high_toxicity$esr * 100,
            target_label))
cat(sprintf("  Low Efficacy     |  %.1f%%    |  ~%s (c_E dominant)\n",
            validation_results$low_efficacy$esr * 100,
            target_label))
cat(sprintf("  Low Immune       |  %.1f%%    |  ~%s (c_I dominant)\n",
...
calibration_summary <- list(
  ...
  target_range = threshold_settings$target_esr_range,
  n_sim_per_candidate = threshold_settings$n_sim_per_candidate,
  validation_n_sim = threshold_settings$validation_n_sim,
...
cat("Target ESR Range:", target_label, "\n")
cat("Quick mode:", quick_mode, "\n")
cat("Simulations per candidate:", threshold_settings$n_sim_per_candidate, "\n")
sink()
