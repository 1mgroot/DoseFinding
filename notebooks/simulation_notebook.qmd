...
This chunk prints Monte Carlo summary results across all simulations. Detailed posterior and allocation plots use the first replicate as an example trial.
...
cat("
--- Example Trial Results (first simulation) ---
")

if (results$terminated_early) {
  cat("Example trial terminated early at stage:", results$termination_stage, "\n")
  cat("Reason:", results$termination_reason, "\n")
  cat("No Optimal Dose selected\n")
} else {
  cat("Example final OD:", results$final_od, "\n")
  cat("Example final utility:", round(results$final_utility, 2), "\n")
  cat("Example PoC validated:", results$poc_validated, "\n")
  cat("Example PoC probability:", round(results$poc_probability, 3), "\n")
  cat("Example selection reason:", results$selection_reason, "\n")
}
...
p_alloc_time <- ggplot(results$all_alloc_probs, aes(x = Stage, y = Prob, color = factor(Dose), group = factor(Dose))) +
...
allocation_counts <- results$all_data %>%
  mutate(
    d = as.integer(d),
...
allocation_summary <- allocation_grid %>%
  left_join(allocation_counts, by = c("d", "stage")) %>%
  mutate(
    n_participants = dplyr::coalesce(n_participants, 0L),
